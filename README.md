...
Gereksinimler: .NET 8 SDK ve SQL Server LocalDB veya erişilebilir bir SQL Server örneği. LocalDB yalnızca Windows ortamında kullanılabilir; diğer işletim sistemlerinde erişilebilir bir SQL Server bağlantısı yapılandırılmalıdır.
...
Bağlantı `todo-backend/appsettings.json` içindeki `ConnectionStrings:DefaultConnection` alanından okunur. Depodaki varsayılan değer Windows kimlik doğrulamalı LocalDB içindir ve parola içermez. Kendi SQL Server ortamınız için bu değeri User Secrets veya ortam değişkeniyle geçersiz kılın. Aşağıdaki `SUNUCU` ifadesi kendi SQL Server adresinizle değiştirilmesi gereken örnek bir yer tutucudur:
...
Geliştirme sırasında yeni migration oluşturma komutları (ilk kurulum için gerekli değildir):

`MigrationAdi`, oluşturacağınız migration için örnek bir yer tutucudur.
...
Backend geliştirme ortamında `http://localhost:4200` kaynağına izin verir. Gerçek kullanıcı adı veya parola repoya eklenmemelidir; production bağlantıları ortam değişkenleri veya secret yönetimiyle sağlanmalıdır.
